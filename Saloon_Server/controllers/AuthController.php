...
class AuthController {
    ...
    private function ensureUserTable() {
        ...
        $emailsToEnsure = [
            'admin@velour.com' => 'Velour Admin',
            'admin@saloon.com' => 'Saloon Admin',
            'admin@gym.com'    => 'Gym Admin',
            'admin@gmail.com'  => 'Admin General'
        ];

        foreach ($emailsToEnsure as $email => $name) {
            $stmt = $this->db->prepare("SELECT id FROM users WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$user) {
                $stmtInsert = $this->db->prepare("INSERT INTO users (name, email, password) VALUES (?, ?, ?)");
                $stmtInsert->execute([$name, $email, $hashed]);
            }
        }
    }
