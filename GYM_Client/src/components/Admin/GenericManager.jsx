...
export default function GenericManager({ title, tableKey, fields, headingSlug, isSingleRow = false }) {
  ...
  return (
    <div className="admin-card" style={{ color: 'var(--text)' }}>
      ...
      {showForm ? (
        <form onSubmit={handleSave}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div key={field.key} className="admin-form-group" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                {field.type === 'textarea' ? (
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    >
                      <div style={{ padding: '6px', background: 'var(--surface)', borderRadius: '4px', border: '1px solid var(--border)', display: 'flex' }}>
                        {(() => {
                          ...
                          return React.createElement(IconComp, { size: 16, color: 'var(--accent)' });
                        })()}
                      </div>
                    ...
                    {activeDropdown === field.key && (
                      <div className="icon-dropdown-grid active">
                        <div className="icon-search-bar">
                          <LucideIcons.Search size={14} color="var(--text3)" />
                          <input 
                            type="text" 
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {imagePreviews[field.key] ? (
                      <div style={{ position: 'relative', width: '120px', height: '120px', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--surface)' }}>
                      ...
                      <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, width: '120px', height: '120px', borderRadius: '4px', border: '1.5px dashed var(--border)', background: 'var(--surface)', cursor: 'pointer' }}>
                        <LucideIcons.UploadCloud size={24} color="var(--text3)" />
                        <span style={{ fontSize: 12, color: 'var(--text3)' }}>Upload Image</span>
                        <input 
                          type="file" 
                          accept="image/*" 
                          style={{ display: 'none' }} 
                    ...
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <LucideIcons.Star
                          ...
                          color={ star <= (Number(formData[field.key]) || 0) ? '#facc15' : 'var(--border)' }
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '16px', background: 'var(--surface)', borderRadius: '6px', border: '1px solid var(--border)' }}>
                    {(Array.isArray(formData[field.key]) ? formData[field.key] : []).map((item, idx) => (
                      <div key={idx} style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'flex-start', background: 'var(--card)', padding: '16px', borderRadius: '4px', border: '1px solid var(--border)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, flex: 1, minWidth: '240px' }}>
                          {(field.schema || []).map(sch => (
                            <div key={sch.key} style={{ display: 'flex', flexDirection: 'column', gap: 4, gridColumn: sch.gridColumn || 'span 2' }}>
                              <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{sch.label}</span>
                              {sch.type === 'icon' ? (
                                <div style={{ position: 'relative' }}>
                                  <div 
                                    className="admin-input admin-icon-selector-trigger" 
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      const dropdownKey = `${field.key}_${idx}_${sch.key}`;
                                      setActiveDropdown(activeDropdown === dropdownKey ? null : dropdownKey);
                                    }}
                                  >
                                    <div style={{ padding: '4px', background: 'var(--surface)', borderRadius: '4px', border: '1px solid var(--border)', display: 'flex' }}>
                                      {(() => {
                                        const iconName = item[sch.key];
                                        const IconComp = (iconName && LucideIcons[iconName]) ? LucideIcons[iconName] : LucideIcons.HelpCircle;
                                        return React.createElement(IconComp, { size: 14, color: 'var(--accent)' });
                                      })()}
                                    </div>
                                    <span style={{ fontSize: 13 }}>{item[sch.key] || 'Select Icon'}</span>
                                    <LucideIcons.ChevronDown size={14} style={{ marginLeft: 'auto' }} />
                                  </div>

                                  {activeDropdown === `${field.key}_${idx}_${sch.key}` && (
                                    <div className="icon-dropdown-grid active">
                                      <div className="icon-search-bar">
                                        <LucideIcons.Search size={14} color="var(--text3)" />
                                        <input 
                                          ...
                                          onKeyUp={(e) => {
                                            ...
                                            items.forEach(itemEl => {
                                              ...
                                              itemEl.style.display = name.includes(term) ? 'flex' : 'none';
                                            });
                                          }}
                                          onClick={(e) => e.stopPropagation()}
                                        />
                                      ...
                                      <div className="icon-grid-scroll">
                                        {Object.keys(LucideIcons)
                                          ...
                                          .map(iconName => (
                                            <div 
                                              key={iconName} 
                                              className={'icon-grid-item ' + (item[sch.key] === iconName ? 'active' : '')}
                                              data-name={iconName}
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                const next = [...(formData[field.key] || [])];
                                                next[idx] = { ...item, [sch.key]: iconName };
                    ...
                    <button 
                      ...
                      style={{ 
                        ...
                        padding: '12px', borderRadius: '4px', border: '1.5px dashed var(--border)', 
                        background: 'var(--card)', color: 'var(--text3)', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                        transition: 'all 0.2s'
                      }}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                          ...
                          {isValObj && 'ok' in val && (
                            <button
                              ...
                              style={{
                                ...
                                background: val.ok ? 'rgba(232, 255, 0, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                                color: val.ok ? 'var(--accent)' : '#ef4444',
                                border: val.ok ? '1px solid var(--border)' : '1px solid rgba(239, 68, 68, 0.2)',
                    ...
                    <button 
                      ...
                      style={{ 
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, 
                        padding: '10px', borderRadius: '4px', border: '1.5px dashed var(--border)', 
                        background: 'var(--surface)', color: 'var(--text3)', fontSize: 13, fontWeight: 600, cursor: 'pointer' 
                      }}
                    >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {(Array.isArray(formData[field.key]) ? formData[field.key] : []).map((val, idx) => {
                      ...
                      return (
                        <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'var(--surface)', padding: 8, borderRadius: '4px', border: '1px solid var(--border)' }}>
                          <select
                            ...
                            className="admin-input"
                            style={{ width: '120px', padding: 8, borderRadius: 4, background: 'var(--card)', fontSize: '12px', fontWeight: 'bold' }}
                          >
                          ...
                          <input 
                            ...
                            style={{ flex: 1, padding: 8, borderRadius: 4, background: 'var(--card)', fontSize: '12px' }}
                          />
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
              ...
              {selectedItem && (
                <div style={{ fontSize: '12px', color: 'var(--accent)', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(232,255,0,0.1)', padding: '6px 12px', borderRadius: '4px', border: '1px solid var(--border)' }}>
        ...
        <>
          ...
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>Loading data...</div>
          ) : data.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text3)', background: 'var(--surface)', borderRadius: '6px', border: '1.5px dashed var(--border)' }}>No records found. Add one to begin.</div>
          ) : (
              ...
              {data.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Header Block */}
                    <div className="admin-info-row" style={{ background: 'var(--surface)', margin: '0', padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', width: '100%', paddingRight: '28px' }}>
                        {imageField && item[imageField.key] ? (
                          <img 
                            ...
                            alt="Preview" 
                            style={{ width: '48px', height: '48px', objectFit: 'cover', borderRadius: '50%', border: '2px solid var(--border)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.3)' }} 
                          />
                        ) : iconField && item[iconField.key] ? (
                          <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(232,255,0,0.1)', border: '2px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.3)' }}>
                            {React.createElement(
                              (item[iconField.key] && LucideIcons[item[iconField.key]]) ? LucideIcons[item[iconField.key]] : LucideIcons.User,
                              { size: 20, color: 'var(--accent)' }
                            )}
                          </div>
                        ) : (
                          <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'var(--border)', border: '2px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.3)' }}>
                            <LucideIcons.Tag size={20} style={{ color: 'var(--text3)' }} />
                          </div>
                        )}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                            <h4 style={{ margin: 0, fontSize: '15px', fontWeight: '800', color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          ...
                          {subField && (
                            <p style={{ margin: 0, fontSize: 12, color: 'var(--accent)', fontWeight: '700', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              ...
                              <div className="admin-info-row" key={f.key} style={{ flexDirection: 'column', alignItems: 'stretch', padding: '12px 20px' }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                  {arr.filter(Boolean).map((tag, idx) => (
                                    <span key={idx} style={{ display: 'inline-block', background: 'var(--surface)', color: 'var(--text2)', border: '1px solid var(--border)', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: '600' }}>
                              ...
                              <div className="admin-info-row" key={f.key} style={{ flexDirection: 'column', alignItems: 'stretch', padding: '12px 20px' }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                  {arr.map((item, idx) => {
                                    ...
                                    return (
                                      <span key={idx} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(232,255,0,0.1)', color: 'var(--accent)', border: '1px solid var(--border)', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: '750' }}>
                              ...
                              <div style={{ display: 'flex', gap: '2px', alignItems: 'center' }}>
                                {[1, 2, 3, 4, 5].map((starNum) => (
                                  <LucideIcons.Star 
                                    ...
                                    color={starNum <= starRating ? '#facc15' : 'var(--border)'} 
                                  />
                                ))}
                                <span style={{ fontSize: '11px', color: 'var(--text3)', fontWeight: '700', marginLeft: '6px' }}>
                                  {starRating}/5
                                </span>
                              </div>
                            </div>
                          ...
                          return (
                            <div className="admin-info-row" key={f.key} style={{ padding: '12px 20px' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                {React.createElement(LucideIcons[displayVal] || LucideIcons.HelpCircle, { size: 14, color: 'var(--accent)' })}
