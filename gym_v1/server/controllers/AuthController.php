...
class AuthController {
    ...
    private function ensureUserTable() {
        ...
        // Explicitly guarantee that both admin@gmail.com and admin@gym.com exist with password 'admin123'
        $hashed = password_hash('admin123', PASSWORD_DEFAULT);
        
        $stmt = $this->db->prepare("SELECT password FROM users WHERE email = 'admin@gmail.com'");
        $stmt->execute();
        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$user) {
            $stmtInsert = $this->db->prepare("INSERT INTO users (name, email, password) VALUES (?, ?, ?)");
            $stmtInsert->execute(['Administrator', 'admin@gmail.com', $hashed]);
        } else {
            if (!password_verify('admin123', $user['password'])) {
                $this->db->prepare("UPDATE users SET password = ? WHERE email = 'admin@gmail.com'")->execute([$hashed]);
            }
        }

        $stmt = $this->db->prepare("SELECT password FROM users WHERE email = 'admin@gym.com'");
        $stmt->execute();
        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$user) {
            $stmtInsert = $this->db->prepare("INSERT INTO users (name, email, password) VALUES (?, ?, ?)");
            $stmtInsert->execute(['Gym Admin', 'admin@gym.com', $hashed]);
        } else {
            if (!password_verify('admin123', $user['password'])) {
                $this->db->prepare("UPDATE users SET password = ? WHERE email = 'admin@gym.com'")->execute([$hashed]);
            }
        }
    }
