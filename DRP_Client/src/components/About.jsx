...
export default function About() {
  ...
  const about = dbAbout && dbAbout.length > 0 ? dbAbout[0] : {
    ...
    desc1: "Dr. Aisha Malik is a board-certified Homoeopathic Physician with over 12 years of clinical experience. She specialises in chronic disease management, women's health, and detoxification through evidence-based Hijama (cupping) therapy.",
    desc2: "Her patient-first approach blends classical homoeopathic case-taking with functional medicine insights, delivering lasting results for complex and long-standing conditions — all through secure video consultations.",
    image: "",
    cards: [
      {
        icon: "GraduationCap",
  ...
  const cards = typeof about.cards === 'string' 
    ? (JSON.parse(about.cards) || []) 
    : (Array.isArray(about.cards) ? about.cards : []);
          ...
          <div className="about-img-wrap">
            {about.image ? (
              <img 
                ...
                alt="Doctor portrait" 
                style={{ width: '200px', height: '250px', objectFit: 'cover', borderRadius: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }} 
              />
            ) : (
        ...
        <div className="about-content reveal reveal-delay-1 visible">
          ...
          <ul className="qual-list">
            {cards.map((card, cardIdx) => {
              const IconComp = (card.icon && LucideIcons[card.icon]) ? LucideIcons[card.icon] : LucideIcons.GraduationCap;
              return (
                <li className="qual-item" key={cardIdx}>
                  <span className="qual-icon" style={{ color: 'var(--accent)', display: 'inline-flex' }}>
                    <IconComp size={22} />
                  </span>
